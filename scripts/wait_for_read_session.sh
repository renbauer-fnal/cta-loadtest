...
while [ $request_queue != "1" ] && [ $nostrict -eq 0 ]; do
        sleep 10
        echo -n "."
        request_queue=$(XrdSecPROTOCOL=sss XrdSecSSSKT=/etc/ctafrontend_client_sss.keytab cta-admin sq | grep "Retrieve ctasfatest${test_type}" | grep -E "[34]\.[0-9]T" | wc -l)
done    
echo ""
echo "Done"

# Active tape to begin reads
...
session=""
echo -n "Waiting for Retrive session to start: "
while [ -z $session ]; do
	sleep 10
	echo -n "."
	session=$(XrdSecPROTOCOL=sss XrdSecSSSKT=/etc/ctafrontend_client_sss.keytab cta-admin dr ls | grep $tape_vid | grep "Transfer" | grep "Retrieve" | awk '{print $14}')
done
echo ""
echo "SESSION IS: $session"
echo "read $session" >> $session_file
